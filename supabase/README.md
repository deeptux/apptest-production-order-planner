...
2. In SQL Editor, run the migrations in order (copy the file contents, not the path):
   ...
   - **003_override_requests_delete_policy.sql** — **required** for “remove from list” / trash on **My requests** when using Supabase. Without it, the app reports success but **0 rows are deleted** and items never disappear. Run this in the SQL Editor after `001` and `002`.
